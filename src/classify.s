...
mult_done1: 
    ...
    slli a0, a0, 2             # Multiply by 4 (size of int)
    jal malloc
    beq a0, x0, error_malloc
    ...
    mv t2, t0       # Copy t0 to t2
    mv t3, t1       # Copy t1 to t3
    beq t0, zero, mult_done2    # If t0 is zero, result is zero
    beq t1, zero, mult_done2
mult_loop2:
    andi t4, t3, 1             # t4 = t3 & 1
    beq t4, zero, skip_add2
    add a1, a1, t2             # a1 += t2
skip_add2:
    slli t2, t2, 1              # t2 <<= 1
    srli t3, t3, 1              # t3 >>= 1
    bne t3, zero, mult_loop2
mult_done2:
    ...
    mv t2, t0       # Copy t0 to t2
    mv t3, t1       # Copy t1 to t3
    beq t0, zero, mult_done3    # If t0 is zero, result is zero
    beq t1, zero, mult_done3
mult_loop3:
    andi t4, t3, 1             # t4 = t3 & 1
    beq t4, zero, skip_add3
    add a0, a0, t2             # a0 += t2
skip_add3:
    slli t2, t2, 1              # t2 <<= 1
    srli t3, t3, 1              # t3 >>= 1
    bne t3, zero, mult_loop3
mult_done3:
    ...
    lw a2, 0(s5)  # load number of rows into third arg
    lw a3, 0(s8)  # load number of cols into fourth arg
    ...
    mv t2, t0       # Copy t0 to t2
    mv t3, t1       # Copy t1 to t3
    beq t0, zero, mult_done4    # If t0 is zero, result is zero
    beq t1, zero, mult_done4
mult_loop4:
    andi t4, t3, 1             # t4 = t3 & 1
    beq t4, zero, skip_add4
    add a1, a1, t2             # a1 += t2
skip_add4:
    slli t2, t2, 1              # t2 <<= 1
    srli t3, t3, 1              # t3 >>= 1
    bne t3, zero, mult_loop4
mult_done4:
